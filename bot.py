...
from holiday_data import check_expiry, load_holiday_data, is_holiday
...
async def send_expiry_update(context: ContextTypes.DEFAULT_TYPE):
    ...
    expiry_info = check_expiry(today, holiday_dict, weekly_expiries, monthly_expiries)
...
def main():
    ...
    today = dt.datetime.now().date()
    if is_holiday(today, holiday_dict):
